...
# Defina uma função para extrair dados do site
extractCountry <- function(url) {
  # Lê a página HTML
  page <- read_html(url)
  
  # Encontre os elementos <a> dentro da estrutura especificada
  countries <- page %>%
    html_nodes(".ipc-metadata-list-item__list-content-item--link[role='button']") %>%
    html_text() %>%
    paste(collapse = ", ")  # Concatena os países separados por vírgulas
  
  return(countries)
}

# Aplica a função à coluna 'url' e armazena o resultado na nova coluna 'country'
dcountry <- dcountry %>%
  rowwise() %>%
  mutate(country = extractCountry(url))
...
# URL do filme no IMDb
url <- "https://www.imdb.com/title/tt1375666/"

# Função para extrair países de origem
extractCountry <- function(url) {
  # Lê a página HTML
  page <- read_html(url)
  
  # Extrai os países de origem
  countries <- page %>%
    html_nodes(xpath = '//*[text()="Países de origem"]/following-sibling::div//a') %>%
    html_text() %>%
    paste(collapse = ", ")
...
# Teste a função com a URL do filme
...
# Adicione a coluna 'country' ao dataframe 'dcountry'
dcountry$country <- sapply(dcountry$URL, extractCountry)
